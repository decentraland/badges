...
export const AMOUNT_OF_LEGENDARY_WEARABLES_REQUIRED = 3
...
export function createLegendaryLookObserver({
  ...
  async function handle(
    ...
    const rareWearablesEquipped = wearablesWithRarity.filter(
      (wearable) => wearable.metadata?.rarity === Rarity.LEGENDARY
    )
